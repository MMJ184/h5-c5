...
export default function Prefetcher({ menuItems }: { menuItems?: RawMenuItem[] | null }) {
	const pageModules = useMemo(() => import.meta.glob('../pages/**/*.tsx') as Record<string, () => Promise<any>>, []);

	const findModuleKey = (componentName: string | undefined) =>
		Object.keys(pageModules).find(
			(k) => !!componentName && (k.endsWith(`/${componentName}.tsx`) || k.endsWith(`/${componentName}.jsx`)),
		);
